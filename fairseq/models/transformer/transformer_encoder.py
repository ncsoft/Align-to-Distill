...
class TransformerEncoderBase(FairseqEncoder):
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        if cfg.alignment_module:
            self.alignment_module = nn.Conv2d(cfg.encoder.layers*cfg.encoder.attention_heads, 24, 1) # 24 equals to the number of teacher attention maps.
        checkpoint = cfg.checkpoint_activations
        if checkpoint:
            offload_to_cpu = cfg.offload_activations
            self.alignment_module = checkpoint_wrapper(self.alignment_module, offload_to_cpu=offload_to_cpu)
        # if we are checkpointing, enforce that FSDP always wraps the
        # checkpointed layer, regardless of layer size
        min_params_to_wrap = cfg.min_params_to_wrap if not checkpoint else 0
        self.alignment_module = fsdp_wrap(self.alignment_module, min_num_params=min_params_to_wrap)
    ...
    def forward(
        ...
        teacher_maps=None,
        self_teacher_maps=None,
        cross_teacher_maps=None
    ):
        """
        Args:
    ...
    def forward_scriptable(
        ...
        if self.alignment_module:
            attn_tensor = self.alignment_module(attn_tensor)
