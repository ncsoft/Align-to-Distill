...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.self_alignment_module=None
        self.cross_alignment_module=None
        if cfg.alignment_module:
            self.self_alignment_module = nn.Conv2d(cfg.decoder.layers*cfg.decoder.attention_heads, 24, 1) # 24 equals to the number of teacher attention maps.
            self.cross_alignment_module = nn.Conv2d(cfg.decoder.layers*cfg.decoder.attention_heads, 24, 1) # 24 equals to the number of teacher attention maps.
    ...
    def extract_features_scriptable(
        ...
        if not all(i is None for i in cross_attn_list):
        ...
        if incremental_state is None:
            if self.cross_alignment_module:
                cross_attn_tensor = self.cross_alignment_module(cross_attn_tensor)
            if self.self_alignment_module:
                self_attn_tensor = self.self_alignment_module(self_attn_tensor)
