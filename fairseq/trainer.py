...
class Trainer(object):
    ...
    def save_outputs(self, filename, outputs):
        outputs = utils.move_to_cpu(outputs)
        if self.should_save_checkpoint_on_current_rank:
            checkpoint_utils.torch_persistent_save(
                outputs,
                filename,
            ...
            try:
                with maybe_no_sync():
                    ...
                    if self.perform_distillation:
                        with torch.no_grad():
                            teacher_output, (teacher_attn_output, teacher_decoder_self_attn_output, teacher_decoder_cross_attn_output, teacher_encoder_states) = self.teacher_model(**sample["net_input"])

                            sample["teacher_encoder_states"] = teacher_encoder_states
                            sample["teacher_decoder_states"] = teacher_output[1]['inner_states']
                            ...
                            sample['teacher_decoder_self_attn_output'] = teacher_decoder_self_attn_output
                            sample['teacher_decoder_cross_attn_output'] = teacher_decoder_cross_attn_output
